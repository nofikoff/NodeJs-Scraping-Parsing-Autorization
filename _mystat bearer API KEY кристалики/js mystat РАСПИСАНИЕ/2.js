    ...
    return requestPromise(
        ...
        .then(
            ...
            function (errorResponse) {
                console.log("Вышли из Promise 1-001 - по REJECT НЕТ КЛЮЧА", errorResponse);
                return new Promise((resolve, reject) => {
        ...
        .then(
            ...
            function (mykey) {
                ...
                return requestPromise(
                    {
                        method: 'GET',
                        // ЗДЕСЬ ДОБАВИТЬ ДИНАМИЧЕСКИ ДАТУ
                        // ЗДЕСЬ ДОБАВИТЬ ДИНАМИЧЕСКИ ДАТУ
                        // ЗДЕСЬ ДОБАВИТЬ ДИНАМИЧЕСКИ ДАТУ
                        // ЗДЕСЬ ДОБАВИТЬ ДИНАМИЧЕСКИ ДАТУ
                        // ЗДЕСЬ ДОБАВИТЬ ДИНАМИЧЕСКИ ДАТУ
                        ...
                        // ЗДЕСЬ ДОБАВИТЬ ДИНАМИЧЕСКИ ДАТУ
                        // ЗДЕСЬ ДОБАВИТЬ ДИНАМИЧЕСКИ ДАТУ
                        url: 'https://msapi.itstep.org/api/v2/schedule/operations/get-month?date_filter=2020-07-18',
                        json: '',
                        type: 'Authorization',
                        value: 'Bearer ' + mykey
                    }
...
async function msg() {
    const msg = await xxxx();
    console.log('Message:', msg);
}

msg(); // Message: 🤡 <-- after 2 seconds JSON
