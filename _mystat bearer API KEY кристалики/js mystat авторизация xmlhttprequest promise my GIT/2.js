...
requestPromise(
    ...
    .then(
        // resolve
        function (responseNoError) {
            console.log("Вышли из Promise 2-001 - по RESOLVE ЕСТЬ КЛЮЧ");
            let mykey = JSON.parse(responseNoError).access_token;
            //console.log('response 001', e); // JSON response
            console.log('API KEY: ', mykey); // JSON response
            ...
            //для передачи ответа = положительного резульатат - возващаем PROMISE / RESOLVE / API KEY
            return new Promise((resolve, reject) => {
                console.log("Выходим из Promise 2-002 - по RESOLVE");
                resolve(mykey);
            });
        },
        //reject
        function (errorResponse) {
            console.log("Вышли из Promise 2-001 - по REJECT НЕТ КЛЮЧА");
            return new Promise((resolve, reject) => {
                //для передачи ответа = ОТРИЦАТЕЛЬНОГО резульатат - возващаем PROMISE / REJECT / сообщение
                reject("2 Не могу прочитать ключ - логин пароль не правильный");
            });
    ...
    .then(
        // resolve
        function (mykey) {
            console.log("Вышли из Promise 2-002 - по RESOLVE ЕСТЬ КЛЮЧ");
        ...
        // reject
        function (errorResponse) {
            console.log("Вышли из Promise 2-002 - по REJECT НЕТ КЛЮЧА");
            console.log(errorResponse);
            return new Promise((resolve, reject) => {
                reject("НЕТ КЛЮЧА");
            });
    ...
    .then(
        function (e) {
            console.log("Вышли из Promise 2-003 - по RESOLVE ЕСТЬ ДАННЫЕ");
            console.log(e);
        },
        function (e) {
            console.log("Вышли из Promise 2-003 - по REJECT НЕТ ДАННЫХ");
        });
